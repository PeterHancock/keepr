...
(function() {
  ...
  JsonDrop = (function() {
    ...
    function JsonDrop(_arg) {
      var key;
      key = _arg.key;
      log("new JSONDrop " + (new Date().getSeconds()) + " " + key);
    ...
    JsonDrop.prototype.load = function(callback, onError) {
      ...
      return this.client.authenticate(function(error, data) {
        var handleError;
        handleError = onError != null ? onError : _this.handleError;
        if (error) {
    ...
    JsonDrop.prototype.handleError = function(error) {
      alert(error);
      return log(error);
    };
  ...
  log = urlParam('__jsondrop-debug__') ? (console.log('JSONDrop logging...'), function() {
    var args;
